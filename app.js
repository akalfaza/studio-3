...
const updatePage = async () => {
  ...
  const fruitsSugar = fruitsArray.filter(fruit=>{
    return fruit.nutritions.sugar > 15;
  })

  const fruitNames = fruitsSugar.map(fruit =>{
    return fruit.name;
  })
  ...
  const addElement = () => {
    ...
    element.innerHTML = fruitNames;
    const element2 = document.getElementById('cs1300-gallery');
